...
abort() {
    local line="Deployment aborted: $1."
    echo -e "\033[31m${line}\033[0m"
    exit 1
}

exec_scalar() {
    # arguments: 1=db_name, 2=statement
    local dbname=$1
    if [ "$ALLOW_MASTER" -eq 0 ] && [ "$dbname" = 'master' ]; then
        dbname=$DATABASE_NAME
    fi
    if ! /opt/mssql-tools/bin/sqlcmd -b -S "$SERVER_NAME","$SERVER_PORT" -U "$SA_USERNAME" -P "$SA_PASSWORD" -d "$dbname" -Q "set nocount on; $2" -W -h-1; then
        abort "Failed to execute scalar"
    fi
}
exec_file() {
    # arguments: 1=file, 2=file_description
    if ! /opt/mssql-tools/bin/sqlcmd -b -S "$SERVER_NAME","$SERVER_PORT" -U "$SA_USERNAME" -P "$SA_PASSWORD" -d "$DATABASE_NAME" -i "$1"; then
        abort "Failed to execute file"
    fi
}
embed_text() {
    echo -e "$1" >> "$script"
...
embed_migration() {
    ...
    for i in "$1"/*; do
        ...
        if [ -d "$i" ] && [ -n "${rev##*[!0-9]*}" ] && ((10#$from < 10#$rev)) && (( 10#$rev <= 10#$to ));then
...
if [[ -s $script ]]; then
    ...
    echo -e '\n==== Running embedded script in transaction ====\n'
    echo "Script: $script..."
    exec_file "$script" "Embedded script"
...
if [ ${dbinit} -eq 1 ] && [ -d "$dynpath" ]; then
    ...
    echo -e '\n==== Running dynamic script ====\n'
    echo "Script: $script..."
    exec_file "$script" "Dynamic script"
    echo -e '\n==== Running dynamic script complete ====\n'
fi
...
if [ -d "$sttpath" ]; then
    ...
    echo -e '\n==== Running static script ====\n'
    echo "Script: $script..."
    exec_file "$script" "Static script"
    echo -e '\n==== Running static script complete ====\n'
fi
